...
| `committee_proposals` | 3 rows in ambiguous `En estudio` state; presentation fields are legacy-required | Add/backfill owner only through a matching `auth.users` row and add `updated_at`. Preserve presentation values but make those redundant fields nullable for canonical guarded writes. Preserve state; change only the default for future rows to `draft`. New voting RPC requires `active`, so legacy rows cannot transition silently. |

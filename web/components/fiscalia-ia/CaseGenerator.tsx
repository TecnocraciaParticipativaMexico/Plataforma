  ...
  return (
    <section className="grid gap-6 lg:grid-cols-12">
      <div className="space-y-5 lg:col-span-5">
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              <label className="text-xs font-bold uppercase tracking-wider text-slate-600">
                Municipio / localidad
                <input value={form.municipality} onChange={(event) => onFormChange({ municipality: event.target.value.slice(0, 120) })} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-[#E4007C] focus:ring-2 focus:ring-[#E4007C]/15" />
              </label>
            </div>
            ...
            <label className="text-xs font-bold uppercase tracking-wider text-slate-600">
              Narrativa libre
              <textarea value={form.narrative} onChange={(event) => onFormChange({ narrative: event.target.value.slice(0, 6000) })} rows={7} className="mt-1 w-full resize-y rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm leading-6 text-slate-900 outline-none focus:border-[#E4007C] focus:ring-2 focus:ring-[#E4007C]/15" />
            </label>

            <label className="text-xs font-bold uppercase tracking-wider text-slate-600">
              Evidencia referenciada
              <input value={form.evidenceReference} onChange={(event) => onFormChange({ evidenceReference: event.target.value.slice(0, 700) })} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-[#E4007C] focus:ring-2 focus:ring-[#E4007C]/15" />
            </label>
            ...
            <button type="button" onClick={onGenerate} disabled={loading} className="rounded-xl bg-[#E4007C] px-5 py-3 text-sm font-black uppercase tracking-wide text-white shadow-sm transition hover:bg-[#BE185D] disabled:cursor-wait disabled:opacity-70">
        ...
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="mb-3 text-sm font-black uppercase text-[#0A4E84]">Registro de evidencia</h3>
          <input ref={inputRef} type="file" multiple onChange={(event) => void handleFiles(event.target.files)} className="block w-full text-xs text-slate-600 file:mr-3 file:rounded-full file:border-0 file:bg-[#0A4E84] file:px-4 file:py-2 file:text-xs file:font-bold file:text-white" />
        ...
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {!structuredCase ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-[#F8FAFC] p-8 text-center">
              ...
              <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-600">Completa los campos y genera un expediente ciudadano estructurado con resumen, cronología, roles, checklist y trazabilidad local.</p>
            </div>
          ) : (
